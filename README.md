...
- Промпты, которые давались чаще всего (примерные формулировки):
	...
	- "Перепиши код бота для совместимости с aiogram v3 (или инструкцию по откату на aiogram v2)."
...
- Дополнительные мелочи и рекомендации:
	- Для устойчивой записи состояния (чтобы не шлёться одинаковые уведомления повторно) можно создать в Supabase таблицу `notified_orders` и делать upsert, либо использовать Redis/в памяти TTL-кеш. В текущей версии README я оставил опцию простой отправки для наглядности; если нужно — могу добавить сохранение и логику дедупликации.
